"""Parser for CoinCreatedV4 events."""
...
class CreatorCoinV4Parser:
    """Parser for CoinCreatedV4 events from creator coin factory contract."""
    ...
    def parse_log(self, log: Dict[str, Any], block_timestamp: datetime) -> Optional[CreatorCoinEvent]:
        ...
        try:
            if not self.can_parse(log):
                logger.warning("Log cannot be parsed by CreatorCoinV4Parser", 
            ...
            logger.debug("Parsing CoinCreatedV4 event",
            ...
            # Create CreatorCoinEvent (reuse same model as it has same structure)
            ...
            logger.info("Successfully parsed CoinCreatedV4 event",
            ...
            logger.error("Failed to parse CoinCreatedV4 event",
                        tx_hash=log.get("transactionHash", {}).hex() if log.get("transactionHash") else None,
    ...
    def parse_logs(self, logs: List[Dict[str, Any]], block_timestamp: datetime) -> List[CreatorCoinEvent]:
        ...
        logger.info("Parsed CoinCreatedV4 events",
                   total_logs=len(logs),
